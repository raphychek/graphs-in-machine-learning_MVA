...
class IncrementalKCenters:
    def __init__(self, labeled_faces, labels, max_num_centroids=50):
        ...
        labeled_faces,_, labels = sorting(labeled_faces,labels,labels)
    ...
    def online_ssl_update_centroids(self, face):
        ...
        if self.centroids.shape[0] >= self.max_num_centroids + 1:
            ...
            """
            dist = np.copy(self.centroids_distances)

            while True:
                i,j = np.unravel_index(np.argmin(dist, axis=None), dist.shape)
                c_1 = self.centroids[i]
                c_2 = self.centroids[j]
    ...
    def online_ssl_compute_solution(self):
        ...
        """
        f = hard_hfs_online_ssl(L,W,self.Y)
...
if __name__ == '__main__':
    online_face_recognition(["raphael","leanna"])
